...
export default function Home() {
  ...
  const mintCryptoDevToken = async (amount) => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Sucessfully minted UNISOME Dev Tokens");
  ...
  const claimCryptoDevTokens = async () => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Sucessfully claimed UNISOME Dev Tokens");
      await getBalanceOfCryptoDevTokens();
      await getTotalTokensMinted();
      await getTokensToBeClaimed();
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Welcome to UNISOME Devs ICO!</h1>
          <div className={styles.description}>
            You can claim or mint UNISOME Dev tokens here
          </div>
          {walletConnected ? (
            <div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
                You have minted {utils.formatEther(balanceOfCryptoDevTokens)} UNISOME
                Dev Tokens
              </div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
